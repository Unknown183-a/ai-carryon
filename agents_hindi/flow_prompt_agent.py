...
def parse_dialogue(response_text):
    """Parse LLM dialogue — handles label+dialogue same line OR next line."""
    ...
    print(f"Hindi parsed dialogue: {lines_out}")
    return lines_out
...
def generate_flow_prompts_hindi(topic, script, num_clips=3):
    ...
    dialogue_prompt = f"""You are a Hinglish YouTube Shorts script writer. Write 3 specific dialogue lines.
...
- Natural slow conversational Hinglish (Hindi + English mix)
- Max 2 sentences per line
- Never paste topic title word for word
- Use natural phrases: "Yaar suno", "Sach mein", "Honestly yaar", "Not gonna lie"

LINE 1 (Hook — specific shocking angle about topic in Hinglish, build curiosity, don't reveal all facts):
LINE 2 (Facts — explain each fact from "{facts_display}" one by one slowly in Hinglish):
LINE 3 (Verdict — specific conclusion about topic + natural Hinglish follow CTA):"""
...
ENVIRONMENT: {background} — laptop now shows relevant tech graphics.
...
ACTION: Continues from previous clip, delivers final verdict in Hinglish: "{lines['LINE 3']}". Turns slightly more toward camera — direct personal connection. Subtle approving nod, natural warm smile. Bold text 'FOLLOW KARO' appears at bottom in clean modern font.
