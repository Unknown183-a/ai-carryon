# agents_hindi/model_invoke_agent_hindi.py
"""
Shared LLM invocation router for the Hindi pipeline.

This is the Hindi-side twin of agents/model_invoke_agent_english.py.
...
def reset_groq_budget():
    ...
    print(
        f"[llm_router:hindi] Reset for new run — Groq budget: {GROQ_MAX_CALLS_PER_RUN} calls, "
    ...
    with _lock:
        if success:
            ...
            if (
                ...
                print(
                    f"[llm_router:hindi] {provider} failed {FAILURE_THRESHOLD}x in a row this run "
...
def _try_groq(prompt, groq_model, temperature):
    ...
    if not under_budget:
        print(f"[llm_router:hindi] Groq budget exhausted ({GROQ_MAX_CALLS_PER_RUN}/run) — skipping to Gemini")
        return None

    with _lock:
        _state["groq_call_count"] += 1
        count_now = _state["groq_call_count"]
    print(f"[llm_router:hindi] Trying Groq ({count_now}/{GROQ_MAX_CALLS_PER_RUN})")
...
def safe_invoke(prompt, groq_model="openai/gpt-oss-120b", temperature=None):
    ...
    if groq_dead and gemini_dead:
        print(
            "[llm_router:hindi] Both Groq and Gemini are dead this run — "
        ...
        if resp is not None:
            return _normalize_response(resp)
        raise RuntimeError(
            f"[llm_router:hindi] Both providers unavailable this run, last-ditch attempt also failed: {err}"
        )
    ...
    if not groq_dead:
        ...
        print("[llm_router:hindi] Groq marked dead this run — routing directly to Gemini")
        resp = _try_gemini(prompt)
        if resp is not None:
            return _normalize_response(resp)
    print(f"[llm_router:hindi] Primary paths failed for this call — last resort: Groq {LAST_RESORT_GROQ_MODEL}")
    resp, err = _run_with_timeout(
        lambda: _get_groq(LAST_RESORT_GROQ_MODEL, temperature).invoke(prompt),
        GROQ_TIMEOUT_SECONDS,
    )
    if resp is not None:
        return _normalize_response(resp)
    raise RuntimeError(f"[llm_router:hindi] All LLM providers failed for this call: {err}")
