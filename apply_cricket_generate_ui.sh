...
with cricket_tab:
    ...
    st.markdown("Finds a recently finished match, writes the recap, generates voice/video, and uploads to YouTube.")
    ...
    st.markdown("---")
    st.markdown("**Step 1 — Find a recently finished match**")
    ...
    if cricket_find_clicked:
        with st.spinner("Checking CricAPI for recently finished matches..."):
            try:
                ...
                cricket_matches = get_finished_matches(limit=5)
    ...
    if cricket_matches:
        ...
        if st.button("✍️ Generate Recap for This Match", type="primary", key="cricket_generate_pipeline_btn"):
            ...
            for k in ["cricket_summary", "cricket_script", "cricket_seo", "cricket_images",
                ...
                st.session_state.pop(k, None)
            try:
                with st.spinner("📚 Researching match scorecard..."):
                    from agents_cricket.research_agent import get_match_summary
                    cricket_summary = get_match_summary(cricket_selected_match)
                if not cricket_summary:
                    st.error("Could not fetch the scorecard for this match. Try another one from the list above.")
                    st.stop()
                ...
                with st.spinner("✍️ Writing recap script..."):
                    ...
                    st.session_state["cricket_script"] = create_cricket_script(cricket_summary)
                ...
                with st.spinner("🎨 Generating background images..."):
                    ...
                    img_paths, img_errors = generate_backgrounds(cricket_summary, num_images=4)
    ...
    if cricket_quick_clicked:
        with st.spinner("Checking for recently finished matches..."):
            try:
                from agents_cricket.trending_agent import get_finished_matches
                quick_matches = get_finished_matches(limit=5)
            except Exception as e:
                st.error(f"Error fetching matches: {e}")
                quick_matches = []
